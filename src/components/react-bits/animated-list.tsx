...
const AnimatedList: React.FC<AnimatedListProps> = ({
  ...
  const [items, setItems] = useState<AnimatedListItem[]>(() => sourceItems);
  const itemIndexRef = useRef(sourceItems.length);
  const intervalRef = useRef<NodeJS.Timeout | null>(null);
  const [isPaused, setIsPaused] = useState(false);
  const displayItems = autoAddDelay > 0 ? items : sourceItems;
  ...
  useEffect(() => {
    ...
    if (autoAddDelay <= 0 || sourceItems.length === 0) {
      return;
    }
    ...
    intervalRef.current = setInterval(() => {
      ...
      setItems((prevItems) => {
        const sourceItem =
          sourceItems[itemIndexRef.current % sourceItems.length];
  ...
  }, [autoAddDelay, maxItems, sourceItems, isPaused]);
